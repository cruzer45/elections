			...
			<table>
				<thead>
					<tr>
						...
						<th><g:message code="candidate.post.label" default="Post" /></th>
					
						<th><g:message code="candidate.team.label" default="Team" /></th>
				...
				<g:each in="${candidateInstanceList}" status="i" var="candidateInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${candidateInstance.id}">${fieldValue(bean: candidateInstance, field: "firstName")}</g:link></td>
					
						<td>${fieldValue(bean: candidateInstance, field: "lastName")}</td>
					
						<td>${fieldValue(bean: candidateInstance, field: "post")}</td>
					
						<td>${fieldValue(bean: candidateInstance, field: "team")}</td>
